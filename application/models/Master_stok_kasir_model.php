...
class Master_stok_kasir_model extends CI_Model
{
 ...
 {
  $hasil = $this->db->query("SELECT tab_barang.nama,temp_master_stok_kasir.alasan, temp_master_stok_kasir.id_s_kasir, temp_master_stok_kasir.nostokkasir, temp_master_stok_kasir.kode_barang, temp_master_stok_kasir.stok, temp_master_stok_kasir.datetime FROM temp_master_stok_kasir INNER JOIN tab_barang ON tab_barang.kode_barang=temp_master_stok_kasir.kode_barang");
  return $hasil->result();
 }

 public function simpan_barang($kode_barang, $stok, $nostokkasir, $image_name, $alasan, $datetime)
 {
  $hasil = $this->db->query("INSERT INTO temp_master_stok_kasir (nostokkasir,kode_barang,stok, qrcode,alasan,datetime)VALUES('$nostokkasir','$kode_barang','$stok','$image_name','$alasan','$datetime')");
  return $hasil;
 }

 public function get_barang_by_kode($id_s_kasir)
 {
  $hsl = $this->db->query("SELECT tab_barang.nama,temp_master_stok_kasir.alasan, temp_master_stok_kasir.id_s_kasir, temp_master_stok_kasir.nostokkasir, temp_master_stok_kasir.kode_barang, temp_master_stok_kasir.stok, temp_master_stok_kasir.datetime FROM temp_master_stok_kasir INNER JOIN tab_barang ON tab_barang.kode_barang=temp_master_stok_kasir.kode_barang
 ...
 public function insert_trans($notrans, $ket, $id_user, $image_name, $tanggal, $datetime)
 {

  //insert into master_stok_kasir
  $q3 = $this->db->query("INSERT into master_stok_kasir (nostokkasir, ket, id_user, qrcode, tanggal, datetime) VALUES ('$notrans', '$ket', '$id_user','$image_name','$tanggal', '$datetime')");
  //insert into master_stok_kasir_detail
  $q4 = $this->db->query("INSERT into master_stok_kasir_detail (nostokkasir, kode_barang, stok, qrcode, alasan, datetime) SELECT nostokkasir, kode_barang, stok, qrcode, alasan, datetime FROM temp_master_stok_kasir WHERE nostokkasir = '$notrans'");
