...
class Stock_opname_model extends CI_Model
{
 ...
 public function simpan_barang($kode_barang, $stok, $harga, $lokasi, $nostockopname, $datetime, $image_name)
 {
  $jumlah = $stok * $harga;
  $hasil  = $this->db->query("INSERT INTO temp_stock_opname (nostockopname,kode_barang,stok, harga, jumlah,lokasi, qrcode ,datetime)VALUES('$nostockopname','$kode_barang','$stok','$harga','$jumlah','$lokasi','$image_name','$datetime')");
  return $hasil;
 }

 public function get_barang_by_kode($id_stock_opname)
 {
  $hsl = $this->db->query("SELECT tab_barang.nama, temp_stock_opname.id_stock_opname, temp_stock_opname.nostockopname, temp_stock_opname.kode_barang, temp_stock_opname.stok, temp_stock_opname.harga,temp_stock_opname.lokasi, temp_stock_opname.datetime FROM temp_stock_opname INNER JOIN tab_barang ON tab_barang.kode_barang=temp_stock_opname.kode_barang
 ...
 public function update_barang($stok, $harga, $lokasi, $id_stock_opname)
 {
  $jumlah = $stok * $harga;
  $hasil  = $this->db->query("UPDATE temp_stock_opname SET stok='$stok', harga='$stok',jumlah='$jumlah',lokasi='$lokasi' WHERE id_stock_opname='$id_stock_opname'");
  return $hasil;
 }
 ...
 public function insert_trans($notrans, $id_user, $ket, $image_name, $tanggal, $sumber, $datetime)
 {
  $this->db->trans_start();
  ...
  $q1 = $this->db->query("INSERT into stock_opname (nostockopname, ket, id_user, jumlah, qrcode, tanggal, sumber, datetime) VALUES ('$notrans','$ket', '$id_user',$q0->jumlah, '$image_name','$tanggal','$sumber', '$datetime')");
  //insert into stock_opname_detail
  $q2 = $this->db->query("INSERT into stock_opname_detail (nostockopname, kode_barang, stok, harga, jumlah,lokasi, qrcode, datetime) SELECT nostockopname, kode_barang, stok, harga, jumlah, lokasi, qrcode, datetime FROM temp_stock_opname WHERE nostockopname = '$notrans'");
